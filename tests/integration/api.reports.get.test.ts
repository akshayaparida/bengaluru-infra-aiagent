...
describe('GET /api/reports (TDD)', () => {
  ...
  it('lists reports sorted by createdAt desc and supports limit', async () => {
    ...
    const r1 = await prisma.report.create({
      data: { description: `${TOKEN} A`, lat: 12.9, lng: 77.5, photoPath: '/tmp/a.jpg', status: ReportStatus.NEW },
    });
    const r2 = await prisma.report.create({
      data: { description: `${TOKEN} B`, lat: 13.0, lng: 77.6, photoPath: '/tmp/b.jpg', status: ReportStatus.NEW },
    });
